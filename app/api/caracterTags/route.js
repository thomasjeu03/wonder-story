...
export async function GET(req) {
    ...
    try {
        ...
        const caracterTags = await prisma.caracterTag.findMany({
            where: parsedWhere,
        ...
        return new Response(JSON.stringify({ caracterTags }), {
            status: 200,
            headers: {
        ...
        console.error("Error while fetching caracterTags from the database", error);
        return new Response(JSON.stringify({ error: 'Failed to fetch caracterTags' }), {
            status: 500,
            headers: {
                'Content-Type': 'application/json',
            },
