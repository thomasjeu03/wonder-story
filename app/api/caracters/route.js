...
export async function GET(req) {
    ...
    try {
        ...
        return new Response(JSON.stringify({ caracters }), {
            status: 200,
            headers: {
        ...
        console.error("Error while fetching caracters from the database", error);
        return new Response(JSON.stringify({ error: 'Failed to fetch caracters' }), {
            status: 500,
            headers: {
                'Content-Type': 'application/json',
            },
